...
const ScheduleAppointment = ({
  ...
  // Scroll to the timeslots when a date is selected
  useEffect(() => {
    if (date) {
  ...
  // Scroll to the book now button when a time slot is selected
  useEffect(() => {
    if (selectedTime) {
  ...
  return (
    <div className="mt-8 grid mx-auto">
      ...
      <div className="flex flex-col max-lg:py-6 max-md:gap-8">
        <div className="flex-1">
          {date ? (
            <AnimationWrapper>
              ...
              {availableTimeSlots ? (
                <AnimationWrapper className="space-y-5 mt-8">
                  <p className="text-sm text-muted-foreground">
                    Available time slots for {moment(date).format("MMM Do YY")}
                  </p>
                  <div className="flex flex-wrap gap-5" ref={timeslotsRef}>
        ...
        {selectedTime?.id && (
          <AnimationWrapper className="space-y-2 mt-10">
            <p className="text-sm text-muted-foreground">
              Selected time and date{" "}
            </p>
            <div
              className="max-w-full flex items-center px-4 py-6 bg-muted dark:bg-muted/40 rounded-md mr-4"
              ref={bookNowRef}
            >
