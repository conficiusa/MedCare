...
export const createAvailability = async (
	...
	try {
		...
		// Get start and end timestamps for the selected date (to find all availability on the same date)
		const startOfDay = new Date(dateOnly);
		const endOfDay = new Date(dateOnly);
		endOfDay.setHours(23, 59, 59, 999);

		// Find availability by doctorId and date (where date falls within the same day)
		...
		if (existingAvailability) {
			...
			// Create new availability with the exact date from the input
